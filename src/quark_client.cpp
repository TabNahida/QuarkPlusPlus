...
QuarkClient::QuarkClient(AppConfig config)
    : config_(std::move(config)),
      http_(config_.request_timeout_ms) {
...
json QuarkClient::api_get_json(const std::string& url,
...
json QuarkClient::api_post_json(const std::string& url,
                                const json& body,
                                const QueryList& query,
                                const HeaderList& headers) const {
    auto merged_headers = api_headers();
...
json QuarkClient::growth_sign() const {
    const auto result = api_post_json(std::string(kGrowthBase) + "/capacity/growth/sign", json::object(), base_params());
...
RemoteEntry QuarkClient::resolve_path(const std::string& remote_path) const {
    ...
    auto current = RemoteEntry {.fid = "0", .name = "/", .parent_fid = "", .size = 0, .dir = true, .raw = json::object()};
    for (const auto& segment : split_remote_path(remote_path)) {
        auto children = list_directory(current.fid);
        const auto it = std::find_if(children.begin(), children.end(), [&](const RemoteEntry& entry) {
...
std::string QuarkClient::get_share_token(const std::string& pwd_id,
                                         const std::optional<std::string>& passcode) const {
    auto query = base_params();
    query.emplace_back("__dt", std::to_string(unix_time_milliseconds_now() % 1000));
    query.emplace_back("__t", std::to_string(unix_time_milliseconds_now()));
...
json QuarkClient::get_share_detail(const std::string& pwd_id,
...
json QuarkClient::share_create(const std::vector<std::string>& fids,
    ...
    const auto password_info = api_post_json(std::string(kApiBase) + "/share/password",
...
json QuarkClient::transfer_share(const std::string& share_url,
...
QuarkClient::UploadPreflight QuarkClient::prepare_upload(const std::filesystem::path& local_path,
    ...
    if (preflight.task_id.empty() || preflight.obj_key.empty() || preflight.bucket.empty() || preflight.upload_url.empty()) {
        throw QuarkException("预上传响应缺少关键字段");
    }
    return preflight;
}

json QuarkClient::check_fast_upload(const UploadPreflight& preflight,
                                    const HashDigests& digests) const {
    json body {
...
std::string QuarkClient::get_upload_auth(const UploadPreflight& preflight,
                                         const std::string& auth_meta) const {
...
std::string QuarkClient::upload_part(const UploadPreflight& preflight,
    ...
    const auto auth_meta = std::format("PUT\n\n{}\n{}\nx-oss-date:{}\nx-oss-user-agent:{}\n/{}/{}?partNumber={}&uploadId={}",
                                       mime_type,
                                       now,
                                       now,
                                       kOssUserAgent,
    ...
    const auto response = http_.upload_file_range(make_oss_object_url(preflight),
                                                  {{"partNumber", std::to_string(part_number)},
                                                   {"uploadId", preflight.upload_id}},
                                                  headers,
                                                  file_path,
                                                  offset,
                                                  length,
...
void QuarkClient::complete_upload(const UploadPreflight& preflight, const std::vector<std::string>& etags) const {
    ...
    auth_meta << "x-oss-date:" << now << '\n'
              ...
              << '/' << preflight.bucket << '/' << preflight.obj_key
              << "?uploadId=" << preflight.upload_id;
...
UploadResult QuarkClient::upload_file(const std::filesystem::path& local_path,
                                      const std::string& pdir_fid,
    ...
    for (std::uint64_t part = 0; part < part_count; ++part) {
        ...
        for (int attempt = 0; attempt < config_.upload_retry_count; ++attempt) {
            try {
                const auto etag = upload_part(preflight,
                                              local_path,
                                              mime_type,
                                              offset,
                                              length,
                                              ...
                                              [&](std::uint64_t current, std::uint64_t) {
                                                  progress_bar.update(uploaded + current, file_size);
                                              });
...
std::unordered_map<std::string, std::string> QuarkClient::request_download_urls(const std::vector<std::string>& fids) const {
    auto query = base_params();
...
void QuarkClient::collect_download_targets_recursive(const RemoteEntry& entry,
...
std::vector<DownloadTarget> QuarkClient::collect_download_targets(const std::string& fid) const {
    ...
    if (fid == "0") {
        collect_download_targets_recursive(RemoteEntry {.fid = "0", .name = "", .parent_fid = "", .size = 0, .dir = true, .raw = json::object()},
                                           {},
                                           output);
        return output;
    }
...
void QuarkClient::download(const std::string& fid,
