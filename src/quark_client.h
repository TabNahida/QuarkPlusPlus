...
public:
    ...
    [[nodiscard]] json share_create(const std::vector<std::string>& fids,
                                    const std::string& title,
                                    int expired_type,
                                    int url_type,
                                    const std::optional<std::string>& passcode) const;
    [[nodiscard]] json share_list(int page, int size) const;
    [[nodiscard]] json transfer_share(const std::string& share_url,
                                      const std::string& to_pdir_fid,
                                      const std::optional<std::string>& passcode) const;

    [[nodiscard]] UploadResult upload_file(const std::filesystem::path& local_path,
    ...
    void download(const std::string& fid,
...
private:
    ...
    [[nodiscard]] json api_get_json(const std::string& url,
                                    const QueryList& query = {},
                                    const HeaderList& headers = {}) const;
    [[nodiscard]] json api_post_json(const std::string& url,
                                     const json& body,
                                     const QueryList& query = {},
                                     const HeaderList& headers = {}) const;
    ...
    [[nodiscard]] std::string get_share_token(const std::string& pwd_id,
                                              const std::optional<std::string>& passcode) const;
    [[nodiscard]] json get_share_detail(const std::string& pwd_id,
                                        const std::string& stoken,
                                        const std::string& pdir_fid) const;
    [[nodiscard]] UploadPreflight prepare_upload(const std::filesystem::path& local_path,
                                                 const std::string& pdir_fid,
                                                 const std::string& file_name) const;
    [[nodiscard]] json check_fast_upload(const UploadPreflight& preflight,
                                         const HashDigests& digests) const;
    [[nodiscard]] std::string get_upload_auth(const UploadPreflight& preflight,
                                              const std::string& auth_meta) const;
    [[nodiscard]] std::string make_oss_object_url(const UploadPreflight& preflight) const;
    [[nodiscard]] std::string upload_part(const UploadPreflight& preflight,
    ...
    [[nodiscard]] std::unordered_map<std::string, std::string> request_download_urls(const std::vector<std::string>& fids) const;
    void collect_download_targets_recursive(const RemoteEntry& entry,
